...
/// Data struct containing information about the device this Glean instace is running in.
///
/// This is provided by the language bindings,
/// each field is related to one of Glean's core metrics.
///
/// `client_id`, `os` and `first_run_date` values are calculated directly in Glean core,
/// thus they are the only ones not present in this struct.
...
impl Context {
    // OS version is required and will never return None
    fn os_version(&self) -> Option<String> {
        Some(self.os_version.as_deref().unwrap_or("Unknown").into())
    }

    fn device_manufacturer(&self) -> Option<String> {
        self.device_manufacturer.as_ref().map(|s| s.to_owned())
    }

    fn device_model(&self) -> Option<String> {
        self.device_model.as_ref().map(|s| s.to_owned())
    }

    // Architecture is required and will never return None
    fn architecture(&self) -> Option<String> {
        Some(self.architecture.as_deref().unwrap_or("Unknown").into())
    }

    // App build is required and will never return None
    fn app_build(&self) -> Option<String> {
        Some(self.app_build.as_deref().unwrap_or("Unknown").into())
    }

    // App display version is required and will never return None
    fn app_display_version(&self) -> Option<String> {
        Some(self.app_display_version.as_deref().unwrap_or("Unknown").into())
    }

    fn app_channel(&self) -> Option<String> {
        self.app_channel.as_ref().map(|s| s.to_owned())
    }

    fn locale(&self) -> Option<String> {
        self.locale.as_ref().map(|s| s.to_owned())
    }

    #[cfg(target_os="android")]
    fn android_sdk_version(&self) -> Option<String> {
        self.android_sdk_version.as_ref().map(|s| s.to_owned())
    }
}
...
pub struct Configuration {
    ...
    /// Context about the device this instance of Glean is running in.
    pub context: Context,
}
...
impl Glean {
    ...
    /// Sets application lifetime core metrics provided by the language binding.
    pub fn initialize_application_specific_core_metrics(&self) {
        if let Some(os_version) = self.context.os_version() {
            self.core_metrics.os_version.set(self, os_version);
        }
        if let Some(device_manufacturer) = self.context.device_manufacturer() {
            self.core_metrics.device_manufacturer.set(self, device_manufacturer);
        }
        if let Some(device_model) = self.context.device_model() {
            self.core_metrics.device_model.set(self, device_model);
        }
        if let Some(architecture) = self.context.architecture() {
            self.core_metrics.architecture.set(self, architecture);
        }
        if let Some(app_build) = self.context.app_build() {
            self.core_metrics.app_build.set(self, app_build);
        }
        if let Some(app_display_version) = self.context.app_display_version() {
            log::error!("Setting app_display_version to {}", app_display_version);
            self.core_metrics.app_display_version.set(self, app_display_version);
        }
        if let Some(app_channel) = self.context.app_channel() {
            self.core_metrics.app_channel.set(self, app_channel);
        }
        if let Some(locale) = self.context.locale() {
            self.core_metrics.locale.set(self, locale);
        }
        #[cfg(target_os="android")]
        if let Some(android_sdk_version) = self.context.android_sdk_version() {
            self.core_metrics.android_sdk_version.set(self, android_sdk_version);
        }
    }
